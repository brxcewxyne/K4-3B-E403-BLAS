...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      ...
      <TopBar title={workflow?.title} sourceCount={sources.length} />
      <nav className="panel-tabs">{(["sources", "chat", "workflow"] as Panel[]).map((item) => <button type="button" key={item} className={panel === item ? "active" : ""} onClick={() => setPanel(item)}>{item === "workflow" ? "Progress" : item[0].toUpperCase() + item.slice(1)}</button>)}</nav>
      <div className={`workspace active-${panel}`}>
        ...
        <ChatPanel messages={messages} thinking={thinking} error={chatError} disabled={!workflow || workflowLoading} sourceCount={sources.length} currentStep={currentStep?.order} onSend={sendMessage} onCitation={openCitation} onAdd={() => setModalOpen(true)} onDismissError={() => setChatError("")} />
        <WorkflowPanel workflow={workflow} completed={completed} selectedId={selectedStepId} loading={workflowLoading} error={workflowError} onSelect={setSelectedStepId} onComplete={completeCurrent} onRetry={() => void buildWorkflow(sources, repository)} />
      </div>
