...
- Create a concise workflow of major user-action stages. Do not create a separate workflow step for every bullet, example, test case, keyword, subsection, or small action. Group closely related actions into one meaningful step and place details in whatToDo/howToDoIt. Prefer approximately 5–10 major steps for a normal lab when supported by the source, but follow source structure when genuinely more phases are required.
